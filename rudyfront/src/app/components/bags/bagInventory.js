angular.module('myApp.bagInventory', [])
    .controller('bagInventoryCtrl', function($scope, spinnerService, bagsHttpService, $rootScope, $state, apiUrl) {
        ...
        $scope.initInventories = function(page) {
            ...
            var date = moment();
            bagsHttpService.bagInventoryData($scope.token).then(function(res) {
              if (res.data.status == 200) {
                $scope.bagInventories = res.data.data.map(function(bi) {
                  ...
                  // var bagName = bi.name;
                  // console.log(bagName);
                  // var sortName = bagName.sort();
                  // console.log(bagName.sort(function(a, b){
        ...
        $scope.showDetail = function (valueObj, bag) {
          ...
          bagsHttpService.bagInventoryDetailMonthly(data, $scope.token).then(function(res) {
            if (res.data.status == 200) {
              var refField = '';
              $scope.refText = '';
