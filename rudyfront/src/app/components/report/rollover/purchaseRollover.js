angular.module('myApp.purchaseRollover', [])
    .controller('purchaseRolloverCtrl', function($scope,
        spinnerService,
        $rootScope,
        ...
        $scope.getRolloverList = (page) => {
            ...
            rolloverHttpServices.getPurchaseRolloverList($scope.myForm, $scope.token).then((objS) => {
        ...
        $scope.createRollover = (valid) => {
            ...
            if (valid) {
                ...
                httpService.createPurchaseConfirmationRollover($scope.rolloverForm, $scope.token).then(function(res) {
                    if (res.data.status == 200) {
                        $scope.getRolloverList();
                        swal("Successful!", "Rollover for purchase confirmation created.", "success");
                    } else {
                        swal("Unsuccessful", res.data.userMessage, "error");    
                    }
                },
                function(error) {
                    swal("Unsuccessful", "Was unable to create rollover purchase confirmation", "error");
                });
