angular.module('myApp.productionRollover', [])
    .controller('productionRolloverCtrl', function($scope,
        spinnerService,
        $rootScope,
        ...
        $scope.getRolloverList = (page) => {
            ...
            rolloverHttpServices.getProductionRolloverList($scope.myForm, $scope.token).then((objS) => {
        ...
        $scope.createRollover = (valid) => {
            ...
            if (valid) {
                ...
                httpService.createRollover($scope.rolloverForm, $scope.token).then(function(res) {
                    if (res.data.status == 200) {
                        $scope.getRolloverList();
                        swal("Successful!", "Rollover for production contract created.", "success");
                    } else {
                        swal("Unsuccessful", res.data.userMessage, "error");    
                    }
                },
                function(error) {
                    swal("Unsuccessful", "Was unable to create rollover production contract", "error");
                });
