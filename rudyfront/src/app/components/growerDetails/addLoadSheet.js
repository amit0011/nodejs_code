angular.module('myApp.addLoadSheet', [])
    .controller('addLoadSheetCtrl',
        function(
            ...
            $scope.type = 'Edit';
            $scope.scaleId = $stateParams.type;
            ...
            $scope.initGrower = (growerId) => {
                if (growerId) {
                    httpService.growerDetails(growerId, $scope.token).then(function(res) {
            ...
            if ($scope.type == 'add') {
                $scope.initGrower($scope.growerId);
            }
            ...
            $scope.getContractList = function() {
                ...
                if ($scope.myForm.commodityId && $scope.myForm.growerId) {
                    ...
                    scaleTicketHttpServices.getContractListByCommodity(data, $scope.token).then(function(res) {
                      $scope.contractList = res.data.status == 200 ? res.data.data.purchaseConfirmation : [];
            ...
            $scope.getTicketNumber = () => {
                scaleTicketHttpServices.getScaleTicketNumber('GrowerLoadSheet', $scope.token).then(function(res) {
                    if (res.data.status == 200) {
                        $scope.myForm.ticketNumber = (Number(res.data.data.loadSheetNumber) + 1).toString();
            ...
            if ($scope.scaleId && $scope.type != 'add') {
                scaleTicketHttpServices.getScaleTicketDetails($scope.scaleId, $scope.token).then(function(res) {
                    if (res.data.status == 200) {
                        ...
                        $scope.myForm.growerId = $scope.myForm.growerId ? $scope.myForm.growerId._id: null;
                        $scope.myForm.gradeId = $scope.myForm.gradeId ? $scope.myForm.gradeId._id: null;
                        $scope.getGrade($scope.myForm.commodityId);
                        $scope.commodityShipAnal($scope.myForm.commodityId, 'Edit');
                        $scope.initGrower($scope.myForm.growerId);
            ...
            $scope.submit = () => {
                ...
                // console.log($scope.myForm.receiptType);
                spinnerService.show("html5spinner");
                if (!$scope.myForm._id) {
                    scaleTicketHttpServices.addGrowerLoadSheet(data, $scope.token).then(function(res) {
                        if (res.data.status == 200) {
                            $scope.goToGrowerDetail(res.data.data.growerId);
                        }
                        spinnerService.hide("html5spinner");
                    });
                } else {
                    ...
                    scaleTicketHttpServices.updateGrowerLoadSheet($scope.myForm, $scope.token).then(function(res) {
                        if (res.data.status == 200) {
                            $scope.goToGrowerDetail(res.data.data.growerId);
                        }
                        spinnerService.hide("html5spinner");
            ...
            $scope.goToGrowerDetail = function(grower) {
              if (!grower) return;
              var growerId = typeof grower === 'object' ? grower._id : grower;
              $state.go('growerDetails', { id: growerId });
            };
        });
