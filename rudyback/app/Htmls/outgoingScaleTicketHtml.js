...
module.exports = {
    ...
    if(data.void){
      imgVoid = `background-image: url(${PDF_WATERMARK_IMG}); background-repeat: repeat-y;background-repeat: no-repeat;background-position: 50% 20%;`;
		...
		var buyerAddress = `${data.buyerId.addresses[0].city} ${data.buyerId.addresses[0].postal} ${data.buyerId.addresses[0].country}`;
		...
		let ticketHtml = () => {
			return `<div style="margin: 4px 0 0 0;padding: 0;width: 100%;">
