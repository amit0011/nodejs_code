...
const {formatDate, roundOff, inWords} = require('../libs/utils');

module.exports = {
	outgoingSeedScaleTicket: data => {
		if (
			data.commodityId &&
		...
		var analysis = "";
		var growerName = data.growerId.farmName || `${data.growerId.firstName} ${data.growerId.lastName}`;
		var unloadWeightInWord = inWords(roundOff(data.unloadWeidht), 3);
		var growerAddress = `${data.growerId.addresses[0].city} ${data.growerId.addresses[0].postal} ${data.growerId.addresses[0].country}`;
		...
		let ticketHtml = () => {
			return `<div style="margin: 4px 0 0 0;padding: 0;width: 100%;float: left;background: #fff;">
				<div style="margin: 0;padding: 0;width: 35%;float: left;background: #fff;">
					<img src="${IMG_URL}" style="margin: 0 2% 0 0;padding: 0;float: left;width: 23%;">
					<div style="margin: 0;padding: 0;width: 75%;float: left;background: #fff;text-align: left;">
				...
				<div style="margin: 0;padding: 0;width: 25%;float: left;background: #fff;text-align: right;">
				...
				<table class="table table-bordered" style="margin-bottom: 0;width: 100%;text-transform:uppercase;">
					<tbody>
						<tr>
							...
							<td style="padding: 0 8px;font-size: 6px;">
								<div style="margin: 0;padding: 0;float: left;width: 60%;color: #000000;font-size: 6px;">${growerName}</div>
							</td>
							<td style="padding: 0 8px;font-size: 6px;">
								<div style="margin: 0;padding: 0;float: left;width: 60%;color: #000000;font-size: 6px;"></div>
								<div style="margin: 0;padding: 0;float: left;width: 40%;text-align:right;color: #000000;"></div>
							...
							<td style="padding: 0 8px;font-size: 6px;">
								...
								<div style="margin: 0;padding: 0;float: left;width: 40%;text-align: right;color: #000000;">${data.contractNumber} - ${data.contractExtra}</div>
							</td>
						</tr>
						...
						<tr>
							<td style="padding: 0 8px;font-size: 6px;">
								...
								<div style="margin: 0;padding: 0;float: left;width: 40%;text-align:right;color: #000000;">${data.unloadWeidht}</div>
							</td>
							<td style="padding: 0 8px;font-size: 6px;color: #000000;">${growerAddress}</td>
							<td style="padding: 0 8px;font-size: 6px;">
			...
			<hr style="margin: 1px 0;padding: 0;width: 100%;float: left;background: #000;height: 1px;">
		...
		var html = `
			<div>
				...
				<div style="min-height:100%;width: 100%; float: left;">
					<div style="min-height:33%;margin: 0;width: 100%;background: #fff;">
						${ticketHtml()}
					</div>
					<div style="min-height:33%;padding:5px 0;margin: 0;width: 100%;background: #fff;border-bottom:1px dashed;border-top:1px dashed;">
						${ticketHtml()}
					</div>
					<div style="min-height:33%;margin: 0;width: 100%;background: #fff;">
						${ticketHtml()}
